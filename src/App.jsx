...
import { 
  ...
  Camera, Image as ImageIcon, Upload, MessageSquare, Share2,
  Euro, Wallet, Home, LogOut, Lock, AlertCircle
} from 'lucide-react';
...
const LoginView = () => {
  ...
  const [info, setInfo] = useState(''); // Pour message succès reset
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      ...
      setInfo("Email de réinitialisation envoyé ! Vérifiez vos spams.");
      setMode('login'); // Retour au login
    } catch (err) {
      setError("Erreur : Email introuvable ou invalide.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <Card className="w-full max-w-md p-8">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-500">
            {mode === 'login' ? 'Connexion sécurisée' : 'Réinitialisation mot de passe'}
          </p>
        </div>
        ...
        {mode === 'login' ? (
          <form onSubmit={handleLogin} className="space-y-4">
             ...
             {info && <div className="bg-green-50 text-green-700 p-3 rounded-lg text-sm flex items-center gap-2"><CheckCircle size={16}/> {info}</div>}
             ...
             <div className="text-right">
                <button type="button" onClick={() => { setMode('reset'); setError(''); setInfo(''); }} className="text-xs text-teal-600 hover:underline">
                  Mot de passe oublié ?
                </button>
             </div>

             <Button type="submit" className="w-full mt-4" disabled={loading} icon={Lock}>
          ...
          <form onSubmit={handleResetPassword} className="space-y-4">
             ...
             <p className="text-sm text-slate-600 mb-4">Entrez votre email pour recevoir un lien de réinitialisation.</p>
             <InputGroup label="Email" type="email" value={email} onChange={e => setEmail(e.target.value)} required />
             
             <Button type="submit" className="w-full mt-4" disabled={loading}>
               {loading ? 'Envoi...' : 'Envoyer le lien'}
             </Button>
             <button type="button" onClick={() => setMode('login')} className="w-full text-center text-sm text-slate-500 mt-4 hover:text-slate-800">
...
export default function AcuFlowApp() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, (u) => {
      // SÉCURITÉ : On rejette les anciennes sessions anonymes
      if (u && u.isAnonymous) {
        signOut(auth); // On déconnecte le "fantôme"
        setUser(null); // On force l'état déconnecté
      } else {
        setUser(u);
      }
      setLoading(false);
...
// --- Sub-Components (Unchanged for brevity, but they are needed in the full file) ---
  ...
  return (
    <div className="space-y-6 md:space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        <Card className="p-3 md:p-4 flex flex-col md:flex-row items-center gap-2 md:gap-4 bg-emerald-50 border-emerald-100 text-center md:text-left"><div className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-emerald-100 text-emerald-600 flex items-center justify-center shrink-0"><Euro size={20}/></div><div><p className="text-[10px] md:text-xs text-emerald-600 font-bold uppercase">CA Total</p><p className="text-lg md:text-2xl font-bold text-slate-800">{totalRevenue} €</p></div></Card>
        <Card className="p-3 md:p-4 flex flex-col md:flex-row items-center gap-2 md:gap-4 bg-amber-50 border-amber-100 text-center md:text-left"><div className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-amber-100 text-amber-600 flex items-center justify-center shrink-0"><Clock size={20}/></div><div><p className="text-[10px] md:text-xs text-amber-600 font-bold uppercase">En attente</p><p className="text-lg md:text-2xl font-bold text-slate-800">{pendingRevenue} €</p></div></Card>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
        <div><h3 className="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2"><CalendarDays size={20} className="text-teal-600"/> Prochains RDV</h3><div className="space-y-3">{upcomingAppointments.map(appt => { const patient = patients.find(p => p.id === appt.patientId); return (<div key={appt.id} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3"><div><div className="font-bold text-slate-800">{new Date(appt.date).toLocaleDateString('fr-FR', {weekday:'short', day:'numeric', month:'short'})}</div><div className="text-teal-600 font-medium flex items-center gap-1 text-sm"><Clock size={14}/> {appt.time}</div><div className="font-bold text-slate-700 cursor-pointer hover:text-teal-600 mt-1" onClick={() => { if(patient) { setSelectedPatient(patient); setView('patientDetail'); } }}>{appt.patientName}</div></div><div className="flex gap-2 w-full sm:w-auto justify-end"><Button variant="icon" title="SMS" onClick={() => patient && sendSMS(patient, appt)} icon={MessageSquare} className="w-10 h-10 p-0 rounded-full" /><Button variant="icon" title="Calendrier" onClick={() => generateICSFile(appt, appt.patientName)} icon={Share2} className="w-10 h-10 p-0 rounded-full" /></div></div>); })}{upcomingAppointments.length === 0 && <div className="p-6 bg-white rounded-xl border border-slate-100 text-slate-400 text-center italic">Aucun rendez-vous.</div>}</div></div>
        <div><h3 className="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2"><History size={20} className="text-teal-600"/> Récents</h3><div className="flex flex-col gap-3">{recentPatients.map(p => (<div key={p.id} onClick={() => { setSelectedPatient(p); setView('patientDetail'); }} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm hover:border-teal-300 cursor-pointer flex items-center gap-4 transition-all"><div className="bg-teal-50 text-teal-700 font-bold w-10 h-10 rounded-full flex items-center justify-center shrink-0">{p.nom.charAt(0).toUpperCase()}</div><div className="overflow-hidden"><h4 className="font-bold text-slate-800 truncate">{p.nom}</h4><p className="text-xs text-slate-500 truncate">{p.lastConsultation ? `${new Date(p.lastConsultation).toLocaleDateString()}` : 'Nouveau'}</p></div><ChevronRight className="ml-auto text-slate-300" size={18} /></div>))}</div></div>
      </div>
    </div>
  );
};
const PatientCard = ({ patient, onClick }) => (
  <div onClick={onClick} className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm hover:shadow-md hover:border-teal-300 cursor-pointer transition-all group">
    <div className="flex justify-between items-start mb-3"><div className="bg-teal-50 text-teal-700 font-bold w-10 h-10 rounded-full flex items-center justify-center text-lg">{patient.nom.charAt(0).toUpperCase()}</div><span className="text-xs font-medium text-slate-400 bg-slate-100 px-2 py-1 rounded-md">{patient.age ? `${patient.age} ans` : 'N/A'}</span></div>
...
const AppointmentForm = ({ patient, onSave, onCancel }) => {
  ...
  return (<Card className="p-6"><form onSubmit={handleSubmit} className="space-y-4"><p className="text-slate-600 mb-4">Patient: <span className="font-bold text-slate-900">{patient.nom}</span></p><div className="grid grid-cols-2 gap-4"><InputGroup label="Date *" type="date" value={date} onChange={e => setDate(e.target.value)} required /><InputGroup label="Heure *" type="time" value={time} onChange={e => setTime(e.target.value)} required /></div><InputGroup label="Note (Optionnel)" placeholder="Ex: Suivi douleur épaule" value={note} onChange={e => setNote(e.target.value)} /><div className="flex justify-end gap-3 mt-6"><Button onClick={onCancel} variant="secondary">Annuler</Button><Button type="submit" icon={CalendarDays}>Confirmer RDV</Button></div></form></Card>)
};
...
const PatientDetail = ({ patient, appointments, onBack, onNewConsultation, onEditConsultation, onDeleteConsultation, onEditPatient, onDeletePatient, onSchedule, onDeleteAppointment, db, appId }) => {
  ...
  return (<div className="space-y-6"><div className="flex items-center justify-between"><Button onClick={onBack} variant="ghost" icon={ArrowLeft}>Retour</Button><div className="flex gap-2"><Button onClick={onDeletePatient} variant="danger" icon={Trash2} className="px-2 md:px-4"><span className="hidden md:inline">Supprimer</span></Button><Button onClick={onEditPatient} variant="secondary" icon={Edit2} className="px-2 md:px-4"><span className="hidden md:inline">Modifier</span></Button></div></div><div className="grid grid-cols-1 lg:grid-cols-3 gap-6"><div className="lg:col-span-2"><Card className="p-6 border-l-4 border-l-teal-500 h-full"><div className="flex flex-col md:flex-row justify-between items-start"><div><h1 className="text-2xl md:text-3xl font-bold text-slate-800">{patient.nom}</h1><div className="flex gap-2 md:gap-4 mt-2 text-slate-500 text-sm flex-wrap"><span className="bg-slate-100 px-2 py-0.5 rounded">{patient.age ? `${patient.age} ans` : 'Age N/A'}</span><span>{patient.profession}</span><span className="hidden md:inline">BAZI: {patient.bazi || '-'}</span></div><div className="md:hidden mt-2 text-sm text-slate-500">BAZI: {patient.bazi || '-'}</div></div><div className="text-left md:text-right text-sm text-slate-500 mt-2 md:mt-0 w-full md:w-auto flex flex-row md:flex-col justify-between md:justify-start border-t md:border-t-0 pt-2 md:pt-0"><p className="font-mono text-slate-700">{patient.telephone}</p><p>Enfants: {patient.nombreEnfants}</p></div></div><div className="mt-4 pt-4 border-t border-slate-100"><h4 className="font-bold text-slate-700 text-sm mb-1">Antécédents :</h4><p className="text-slate-600 text-sm">{patient.antecedents || "Aucun antécédent noté."}</p></div></Card></div><Card className="p-5 flex flex-col"><div className="flex justify-between items-center mb-4"><h3 className="font-bold text-slate-700 flex items-center gap-2"><CalendarDays size={18}/> Rendez-vous</h3><Button onClick={onSchedule} variant="primary" className="text-xs px-3 py-1">Planifier</Button></div><div className="flex-1 overflow-y-auto space-y-2 max-h-[150px]">{appointments.length === 0 && <p className="text-sm text-slate-400 italic">Aucun RDV futur.</p>}{appointments.map(appt => (<div key={appt.id} className="text-sm border-b border-slate-100 pb-2 last:border-0 flex justify-between items-center group gap-2"><div className="flex-1"><div className="font-bold text-teal-700">{new Date(appt.date).toLocaleDateString()} à {appt.time}</div><div className="text-slate-500 text-xs">{appt.note}</div></div><div className="flex gap-1"><button onClick={() => sendSMS(patient, appt)} title="Rappel SMS" className="p-1.5 text-slate-400 hover:text-teal-600 hover:bg-slate-100 rounded-full transition-colors"><MessageSquare size={14} /></button><button onClick={() => generateICSFile(appt, patient.nom)} title="Ajouter au calendrier" className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-slate-100 rounded-full transition-colors"><Share2 size={14} /></button><button onClick={() => onDeleteAppointment(appt.id)} title="Supprimer" className="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-full transition-colors"><X size={14} /></button></div></div>))}</div></Card></div><div><div className="flex justify-between items-end mb-4"><h3 className="text-xl font-bold text-slate-800">Historique des Consultations</h3><Button onClick={onNewConsultation} variant="primary" icon={Plus} className="md:px-4"><span className="hidden md:inline">Nouvelle Consultation</span><span className="md:hidden">Nouvelle</span></Button></div><div className="space-y-4">{history.map((consult, idx) => (<div key={consult.id} className="flex gap-4"><div className="flex flex-col items-center"><div className="w-8 h-8 rounded-full bg-teal-100 text-teal-700 flex items-center justify-center font-bold text-sm z-10 border-2 border-white shadow-sm shrink-0">{history.length - idx}</div>{idx !== history.length - 1 && <div className="w-0.5 bg-slate-200 flex-1 my-1"></div>}</div><ConsultationSummaryCard consult={consult} onEdit={() => onEditConsultation(consult)} onDelete={() => onDeleteConsultation(consult.id)} /></div>))}{history.length === 0 && <div className="text-center py-10 bg-white rounded-xl border border-dashed border-slate-300 text-slate-500">Aucune consultation enregistrée.</div>}</div></div></div>);
};
const ConsultationSummaryCard = ({ consult, onEdit, onDelete }) => {
  const [expanded, setExpanded] = useState(false); const obs = consult.observation || {}; const billing = consult.billing || {}; const hasTongueImage = obs.tongueImage && obs.tongueImage.startsWith('data:image');
  return (<Card className="flex-1 mb-2 hover:border-teal-300 transition-colors"><div className="p-4 cursor-pointer flex justify-between items-center group" onClick={() => setExpanded(!expanded)}><div className="flex-1"><div className="flex flex-col md:flex-row md:items-center gap-1 md:gap-3"><div className="flex items-center gap-2"><span className="font-bold text-slate-800">{new Date(consult.date).toLocaleDateString('fr-FR', { weekday: 'short', year: 'numeric', month: 'short', day: 'numeric' })}</span><span className="text-xs bg-slate-100 text-slate-500 px-2 py-1 rounded-full">{new Date(consult.date).toLocaleTimeString('fr-FR', {hour: '2-digit', minute:'2-digit'})}</span></div><div className="flex gap-2">{hasTongueImage && <span className="flex items-center gap-1 text-xs text-teal-600 border border-teal-200 px-2 py-0.5 rounded-full bg-teal-50"><ImageIcon size={10}/> Photo</span>}{billing.price && <span className={`flex items-center gap-1 text-xs border px-2 py-0.5 rounded-full ${billing.status === 'paid' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-amber-50 text-amber-700 border-amber-200'}`}><Euro size={10}/> {billing.status === 'paid' ? 'Payé' : 'En attente'}</span>}</div></div><p className="text-teal-700 font-medium mt-1 text-sm md:text-base">{consult.motif || "Motif non spécifié"}</p></div><div className="flex items-center gap-3"><div className="flex gap-1 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity mr-2" onClick={(e) => e.stopPropagation()}><button onClick={onEdit} className="p-2 text-slate-400 hover:text-teal-600 hover:bg-slate-100 rounded-full" title="Modifier"><Edit2 size={16} /></button><button onClick={onDelete} className="p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-full" title="Supprimer"><Trash2 size={16} /></button></div><ChevronDown className={`text-slate-400 transition-transform ${expanded ? 'rotate-180' : ''}`} /></div></div>{expanded && (<div className="px-4 pb-4 pt-0 border-t border-slate-50 text-sm grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">{(hasTongueImage || obs.tongueTags) && (<div className="col-span-1 md:col-span-2 bg-slate-50 rounded-lg p-3 border border-slate-100 flex flex-col sm:flex-row gap-4">{hasTongueImage && <div className="shrink-0 self-center sm:self-start"><img src={obs.tongueImage} alt="Langue" className="w-24 h-24 object-cover rounded-md border border-slate-300 shadow-sm" /></div>}<div className="flex-1"><h4 className="font-bold text-teal-700 mb-1 flex items-center gap-2"><ImageIcon size={14}/> Diagnostic Langue</h4><div className="flex flex-wrap gap-1 mb-2">{obs.tongueTags && Object.entries(obs.tongueTags).map(([key, value]) => (value && <span key={key} className="text-xs bg-white border border-slate-200 text-slate-600 px-2 py-0.5 rounded shadow-sm">{value}</span>))}</div><p className="text-slate-600 italic">{obs.langueDetail}</p></div></div>)}<div><h4 className="font-bold text-slate-700 mb-1 flex items-center gap-1"><Brain size={14}/> Diagnostic Zang Fu</h4><ul className="list-disc pl-4 text-slate-600 space-y-1">{consult.symptoms && Object.keys(consult.symptoms).map(k => (consult.symptoms[k]?.length > 0 && <li key={k}><span className="font-semibold">{k}:</span> {consult.symptoms[k].join(", ")}</li>))}{(!consult.symptoms || Object.keys(consult.symptoms).length === 0) && <li className="text-slate-400 italic">Rien à signaler</li>}</ul></div><div><h4 className="font-bold text-slate-700 mb-1 flex items-center gap-1"><Activity size={14}/> Traitement</h4><p className="text-slate-600 mb-2"><span className="font-semibold">Principe:</span> {consult.principeTraitement || '-'}</p><div className="bg-teal-50 p-2 rounded text-teal-800 font-mono text-xs break-words">{consult.pointsAcupuncture || "Aucun point noté"}</div></div><div className="col-span-1 md:col-span-2 bg-yellow-50 p-3 rounded border border-yellow-100 text-yellow-800"><span className="font-bold">Notes:</span> {consult.notes}</div>{billing.price && (<div className="col-span-1 md:col-span-2 flex items-center justify-between border-t border-slate-100 pt-3 mt-1"><div className="flex items-center gap-2 text-slate-600"><Wallet size={16} /> <span>{billing.price} € - {billing.method === 'especes' ? 'Espèces' : billing.method === 'cb' ? 'Carte Bancaire' : billing.method === 'cheque' ? 'Chèque' : 'Autre'}</span></div><div className={`font-bold uppercase text-xs px-2 py-1 rounded ${billing.status === 'paid' ? 'text-emerald-700 bg-emerald-100' : 'text-amber-700 bg-amber-100'}`}>{billing.status === 'paid' ? 'Réglé' : 'À régler'}</div></div>)}</div>)}</Card>);
};
